...
const ReelsUI: React.FC = () => (
     <div className="absolute inset-0 text-white pointer-events-none text-xs">
        <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/50 to-transparent">
            ...
            <p className="mb-2 text-sm">Age Smarter This Summer</p>
...
const UnsafeZonesOverlay: React.FC<{ safeZone: Placement['safeZone'] }> = ({ safeZone }) => (
    <>
        <div className="absolute top-0 left-0 right-0 bg-yellow-400/30 pointer-events-none z-30" style={{ height: safeZone.top }}></div>
        <div className="absolute bottom-0 left-0 right-0 bg-yellow-400/30 pointer-events-none z-30" style={{ height: safeZone.bottom }}></div>
        {safeZone.left && <div className="absolute top-0 bottom-0 left-0 bg-yellow-400/30 pointer-events-none z-30" style={{ width: safeZone.left }}></div>}
        {safeZone.right && <div className="absolute top-0 bottom-0 right-0 bg-yellow-400/30 pointer-events-none z-30" style={{ width: safeZone.right }}></div>}
    </>
);
...
export const AdPreview: React.FC<AdPreviewProps> = ({ creativeSet, placement, size = 'large', criticalSummary, onClick }) => {
    ...
    const aspectRatioClass = getAspectRatioClass(placement.aspectRatios[0]);
    ...
    const PhoneSimulation: React.FC<{scaleClass: string}> = ({scaleClass}) => (
        <div className={`relative mx-auto bg-gray-900 border-gray-700 shadow-xl border-[6px] md:border-[10px] rounded-[1.5rem] md:rounded-[2.5rem] ${scaleClass}`}>
            <div className="w-full h-full overflow-hidden bg-brand-bg rounded-[1.2rem] md:rounded-[2rem]">
              {creativeToDisplay ? (
                <div className={`relative w-full h-full flex items-center justify-center overflow-hidden`}>
                  <div className={`relative ${aspectRatioClass} w-full bg-black`}>
                     <UIOverlay placement={placement}>
                         <CreativeContent creative={creativeToDisplay} areRatiosMismatched={areRatiosMismatched} />
                     </UIOverlay>
                     <UnsafeZonesOverlay safeZone={placement.safeZone} />
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-full text-center p-4">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-10 md:h-16 w-10 md:w-16 text-brand-border" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clipRule="evenodd" /></svg>
                  <p className="mt-2 md:mt-4 text-brand-text-secondary text-sm">Sube un archivo</p>
                </div>
              )}
            </div>
        </div>
    ...
    if (size === 'large') {
        return <PhoneSimulation scaleClass="w-[350px] h-[700px]"/>;
    }

    return (
        <div 
            className="flex flex-col h-full bg-brand-surface rounded-lg shadow-lg group cursor-pointer hover:ring-2 hover:ring-brand-primary transition-all"
            onClick={onClick}
        >
            <div className="p-4">
                <p className="text-center text-brand-text font-semibold text-base mb-3">
                    {placement.platform} - {placement.name}
                </p>
                <div className="w-full aspect-[9/18]">
                    <PhoneSimulation scaleClass="w-full h-full"/>
                </div>
            </div>
            {criticalSummary && (
                <div className="px-4 py-3 mt-auto border-t border-brand-border/50 text-center">
                    <ul className="space-y-1">
                        {criticalSummary.map((point, index) => (
                           <li key={index} className="text-brand-text-secondary text-sm">
                            {point}
                           </li>
                        ))}
                    </ul>
